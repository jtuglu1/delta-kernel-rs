...
- `src/alloc_stats.rs` -- allocation-tracking global allocator and native-heap FFI getters
  (`alloc-tracking`)
...
- `tracing`
- `alloc-tracking` -- installs `TrackingAlloc<PeakAlloc>` as the global allocator; enables
  meaningful live, peak, and cumulative allocated/freed `*_native_bytes` getters plus
  `alloc_tracking_enabled` (cdylib only; conflicts with
