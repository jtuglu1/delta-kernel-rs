...
impl FfiSetTransactionArray {
    unsafe fn try_to_kernel(&self) -> DeltaResult<Vec<SetTransaction>> {
        unsafe { raw_slice(self, self.ptr, self.len, "set-transaction array") }?
            .iter()
            .map(|value| unsafe { value.try_to_kernel() })
            .collect()
    }
...
impl FfiSetTransactionState {
    pub(crate) unsafe fn try_to_kernel(&self) -> DeltaResult<SetTransactionState> {
        let transactions = unsafe { self.transactions.try_to_kernel() }?;
        match self.kind {
...
impl FfiDomainMetadataArray {
    unsafe fn try_to_kernel(&self) -> DeltaResult<Vec<DomainMetadata>> {
...
impl FfiDomainMetadataState {
    pub(crate) unsafe fn try_to_kernel(&self) -> DeltaResult<DomainMetadataState> {
        let domain_metadata = unsafe { self.domain_metadata.try_to_kernel() }?;
        match self.kind {
